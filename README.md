...
| `history_path` | путь к CSV с историей операций: наблюдаемые конверсии для стратегии `conversion`, порогов симуляции и рекомендаций | **да** |
...
Настраивается слой одним понятным числом — с какого израсходованного процента дневного
лимита он начинает придерживать провайдера:

```yaml
goals:
  budget_headroom: { activates_at_spent_pct: 90 }   # вмешиваемся с 90% лимита
```

`70` — раньше и осторожнее, `100` — фактически выключено. Внутри процент один раз
переводится в порог по той же кривой ψ, поэтому «ψ ниже порога» буквально означает
«потрачено больше заданного». В объяснении печатаются обе величины:

```
payflow psi 0.033 (ниже порога на 0.062) -> с 1 на 2;
порог: вмешиваемся с 90% израсходованного лимита (psi 0.095)
```
