...
# П5 (docs/plans/P6/P5_readme_защита.md, раздел «Демо ретрая»): на боевом
# seed 42 публичная очередь не содержит ни одного `rejected` -- жюри не увидит
# ни повторной попытки, ни `next_in_cascade`. README §6 несёт команду с этим
# же SEED, подобранным перебором `1..99` по возрастанию (первое значение, где
# `next_in_cascade` появляется). Если кто-то поменяет число в README, не
# поменяв здесь, — спек и README разойдутся, и это будет замечено, а не
# тихо забыто.
#
