  ...
  "paths": {
    ...
    "/analytics/overview": {
      "get": {
        ...
        "summary": "Сводка исходов, длины каскада и временных рядов",
        "description": "То, чего нет в `/report`: гистограмма `simulated_result`, воронка длины\nкаскада и разбивка по корзинам времени. `/report` отвечает на вопрос\n«куда ушёл трафик и почему» и ни одной отметки времени не содержит —\nдинамику по нему не построить.\n\nФильтры те же, что у `/report` и `/decisions`, и применяются так же —\nкак WHERE до агрегации. Пустая выборка возвращает нули и корзины без\nотметок времени, а не 4xx.\n\nОкно временных рядов определяется данными: `from` — самое раннее\nрешение выборки, `to` — самое позднее. Ширина корзины\n(`bucket_seconds`) считается от окна, а не задаётся снаружи.\n",
        ...
        "parameters": [
          ...
          },
          {
            "name": "buckets",
            "in": "query",
            "description": "Число корзин временного ряда (2..96, дефолт 12)",
            "schema": {
              "type": "integer",
              "minimum": 2,
              "maximum": 96,
    ...
    "/analytics/decisions": {
      "get": {
        ...
        "summary": "Решения вместе с контекстом операции",
        "description": "То же, что `GET /decisions`, плюс поля операции, по которым решение\nпринималось: `id`, `created_at`, `amount`, `bank`, `card_brand`,\n`merchant`, `gate`.\n\nОтдельный путь, а не расширение `/decisions`: форма элемента там\nзафиксирована как `routing_decisions_test.json[i]` и обрастать полями\nне должна.\n",
        "operationId": "listDecisionsWithContext",
        "parameters": [
          {
            "$ref": "#/components/parameters/SinceParam"
          },
        ...
        "responses": {
          "200": {
            "description": "Список решений с контекстом",
            "content": {
              "application/json": {
                "schema": {
                  "$ref": "#/components/schemas/ContextDecisionsListResponse"
                }
              }
            }
          },
    ...
    "/capabilities": {
      "get": {
        ...
        "summary": "Допустимые значения для POST /config",
        "description": "Имена стратегий и слоёв берутся из реестров `Routing::Strategies` и\n`Routing::Layers`, поэтому список нигде не дублируется: новый файл в\nкаталоге стратегий сразу виден здесь.\n",
    ...
    "/console/": {
      "get": {
        ...
        "summary": "Routing Console",
        "description": "Пульт движка: обзор долей, симулятор операций, аналитика и контекст.\nСтатика из `dashboard/` (путь в `service.dashboard_path`), данные берёт\nиз этих же ручек — своего состояния у страницы нет.\n",
      ...
      "AnalyticsOverview": {
        ...
        "properties": {
          "total": {
            "type": "integer",
            "minimum": 0,
            "description": "Решений в выборке",
            "example": 10
          },
          "outcomes": {
            "type": "object",
            "description": "Счётчики `simulated_result`. Ключи присутствуют всегда, даже с нулями.",
            "required": [
              "approved",
              "rejected",
              "expired",
          ...
          "attempt_histogram": {
            "type": "array",
            "description": "Воронка каскада: сколько решений дошло до попытки N. Первая строка\nравна числу решений с хотя бы одной попыткой.\n",
            "items": {
              "$ref": "#/components/schemas/AttemptHistogramEntry"
            }
          },
          "approved_count": {
            "type": "integer",
          ...
          "approved_amount": {
            "type": "integer",
            "minimum": 0,
            "description": "Сумма amount по approved",
            "example": 385800
          },
          "avg_latency_sec": {
            "type": "number",
            "nullable": true,
            "description": "Средний latency_sec по approved; null, если approved нет",
            "example": 39.5
          },
          "timeline": {
      ...
      "Timeline": {
        ...
        "properties": {
          "from": {
            ...
            "format": "date-time",
            "nullable": true,
            "description": "Время самого раннего решения выборки; null на пустой выборке",
            "example": "2026-07-30T09:05:00Z"
          },
          "to": {
            "type": "string",
            "format": "date-time",
            "nullable": true,
            "example": "2026-07-30T09:50:00Z"
          },
          "bucket_seconds": {
            "type": "integer",
            "nullable": true,
            "minimum": 1,
            "description": "Ширина корзины; null на пустой выборке",
            "example": 450
          },
          "buckets": {
            "type": "array",
      ...
      "TimelineBucket": {
        ...
        "properties": {
          "at": {
            ...
            "format": "date-time",
            "nullable": true,
            "description": "Левая граница корзины; null на пустой выборке"
          },
          "total": {
            "type": "integer",
            "minimum": 0,
          ...
          "by_provider": {
            "type": "object",
            "description": "Решения корзины по итоговому `selected_provider`.",
            "additionalProperties": {
              "type": "integer",
              "minimum": 0
            },
          ...
          "approved_by_provider": {
            "type": "object",
            "description": "Из них с `simulated_result = approved` — числитель approve rate.",
            "additionalProperties": {
              "type": "integer",
              "minimum": 0
            },
          ...
          "attempts_by_provider": {
            "type": "object",
            "description": "Все попытки корзины, включая skipped, — пульс провайдера.",
            "additionalProperties": {
              "type": "integer",
              "minimum": 0
            },
          ...
          },
          {
            ...
            "properties": {
              "id": {
                "type": "integer",
                "minimum": 1,
                "description": "Порядковый id строки в SQLite",
                "example": 1
              },
              "created_at": {
                "type": "string",
      ...
      "ContextDecisionsListResponse": {
        ...
        "properties": {
          "items": {
            "type": "array",
            "items": {
              "$ref": "#/components/schemas/ContextDecision"
            }
          },
          "total": {
            "type": "integer",
