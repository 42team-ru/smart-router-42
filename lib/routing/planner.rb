...
module Routing
  ...
  class Planner
    ...
    # применяется, только если после него в пуле остаётся хотя бы один внешний
    # кандидат -- иначе оно тихо разошлось бы с эталонным допуском валидатора
    # организаторов, который про интенсивность не знает. Если трактовка
    # изменится на строгую, развилка снимается одной правкой: return
    # [ok_providers, ...] без ветки "пул опустел".
