...
module Routing
  module Constraints
    # Отсекает провайдера, чей процент выше маржи мерчанта: каждая выплата
    # через него убыточна. Исключение делает явный флаг
    # allow_negative_agreement — убыточный канал иногда держат ради оборота.
    #
    # Сравнение через Rational, не Float: в JSON проценты десятичные, Float их
    # не представляет точно, и равные по смыслу значения могут разойтись на
    # последнем бите — провайдер начнёт то проходить, то нет.
    class Margin < Base
