...
module Routing
  module Strategies
    ...
    # Боевая стратегия проекта.
    #
    # Метод делителей Сент-Лагю: заявку получает провайдер с наибольшим
    # вес / (2 × выдано + 1). Наивный «отдать отстающему по проценту» на
    # коротких очередях скачет — после первой заявки у одного 100%, у остальных
    # ноль. Сент-Лагю даёт ближайшее к целевому распределение на каждом шаге,
    # а не только в пределе.
    #
    # Сравнение — перекрёстным умножением целых, без деления: веса в базисных
    # пунктах. При равных значениях побеждает больший вес, при равных весах —
    # имя, чтобы порядок был однозначным.
