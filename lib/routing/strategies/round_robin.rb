...
module Routing
  module Strategies
    # Раздаёт заявки по кругу, никого не выделяя: провайдеры по алфавиту,
    ...
    #
    # В бою не используется намеренно. Нужна как точка отсчёта: без неё
    ...
    # отклонения от достижимого распределения, count_share даёт 0.
    #
    # Требование, которого нет у остальных стратегий: объект обязан быть без
    # памяти. Смещение берётся из счётчика прогона, а не из поля внутри
